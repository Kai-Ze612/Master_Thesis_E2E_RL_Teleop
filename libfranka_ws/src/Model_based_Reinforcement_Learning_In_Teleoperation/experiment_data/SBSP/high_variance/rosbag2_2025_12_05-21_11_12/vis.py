...
def read_rosbag2(bag_path, max_duration_sec=50):
    # CHANGED: Added explicit path check before attempting to open
    bag_path_obj = Path(bag_path).resolve()
    bag_path = str(bag_path_obj)
    
    if not bag_path_obj.exists():
        print(f"\nCRITICAL ERROR: The path '{bag_path}' does not exist.")
        print("Please check for typos in the folder name (e.g., 'Low_delay' vs 'low_delay').")
        return None
