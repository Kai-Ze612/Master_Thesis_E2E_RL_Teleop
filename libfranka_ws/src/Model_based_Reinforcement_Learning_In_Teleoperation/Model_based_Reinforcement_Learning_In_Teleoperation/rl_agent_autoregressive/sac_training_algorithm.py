...
from Model_based_Reinforcement_Learning_In_Teleoperation.rl_agent_autoregressive.sac_policy_network import Actor, Critic
...
from Model_based_Reinforcement_Learning_In_Teleoperation.config.robot_config import (
    ...
    OBS_DIM,
    MAX_EPISODE_STEPS,
    LSTM_MODEL_PATH # Used for validation env creation only
)
...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size: int, device: torch.device):
        ...
        self.action_dim = N_JOINTS # 7D (Torque only)
...
class SACTrainer:
    """Soft actor-critic (SAC) training algorithm."""
    ...
    def __init__(self, 
        ...
        # Create validation environment (Needs LSTM path passed explicitly)
        ...
        # State dim = 112D (Full Observation from Env)
        state_dim = OBS_DIM

        # Initialize Actor and Critic networks
        self.actor = Actor(state_dim=state_dim, action_dim=N_JOINTS).to(self.device)
        self.critic = Critic(state_dim=state_dim, action_dim=N_JOINTS).to(self.device)
        ...
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=SAC_LEARNING_RATE)
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=SAC_LEARNING_RATE)

        # Automatic Entropy Tuning
        if SAC_TARGET_ENTROPY == 'auto':
            self.target_entropy = -float(N_JOINTS) # heuristic
        else:
    ...
    def select_action(self, obs: np.ndarray, deterministic: bool = False) -> np.ndarray:
        ...
        The Env has already handled the LSTM prediction and embedded it in 'obs'.
        """
        with torch.no_grad():
    ...
    def validate(self) -> float:
        ...
        for episode in range(SAC_VAL_EPISODES):
            ...
            for step in range(MAX_EPISODE_STEPS):
                # Reshape obs for batch inference (1, 112)
    ...
    def update_policy(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            q_target = batch['rewards'] + (1.0 - batch['dones']) * SAC_GAMMA * (q_target_min - self.alpha * next_log_probs_t)
    ...
    def train(self, total_timesteps: int):
        ...
        logger.info("="*70)
        logger.info("Starting SAC Training (Environment-Centric Prediction)")
        logger.info(f"  Input Dim: {OBS_DIM} | Output Dim: {N_JOINTS}")
        logger.info("="*70)
        ...
        for t in range(int(total_timesteps // self.num_envs)):
            ...
            # 2. Step Environment (Env handles Prediction internally)
