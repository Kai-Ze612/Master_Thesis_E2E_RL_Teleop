...
franka::RobotState RobotSim::populateFrankaState(){
  ...
  */
  ...
  for(int i=0; i<kNumberOfJoints; i++){
    ...
    k_f_ext_hat_k
    o_f_ext_hat_k 
    tau_ext_hat_filtered - this is used for k_f_ext and o_f_ext

    m_ee
    o_t_ee
    */

  // Calculate tau_ext_hat_filtered and O_F_ext_hat, depending on if identification is active or not
  if(this->has_identification_){
    try{

      Eigen::Map<Eigen::VectorXd>(current_state_.tau_ext_hat_filtered.data(), 7) = this->obs_->get_tau_ext_hat_filtered(current_state_);
      Eigen::Map<Eigen::VectorXd>(current_state_.O_F_ext_hat_K.data(), 6) = this->obs_->get_O_F_ext_hat_K(current_state_, getModel());
    }
    catch(std::exception &e){
      std::cerr << "Calling get_tau_ext_hat_filtered and/or get_O_F_ext_hat_K has failed. Returning default values. Error: " << e.what() << std::endl;
      this->has_identification_ = false;
    }
  }
  if(!this->has_identification_){
...
bool RobotSim::initializeIdentification(std::vector<double> Xb, double mountRho, double mountPhi){
  std::cout << std::endl;
  if(Xb.size() != 66U){
    std::cerr << "Identification parameter Xb has != 66 elements! Received: " << Xb.size() << " Skipping identification initialization." << std::endl;
    return false;
  }
  if(abs(mountRho) > M_PI+1e-3 || abs(mountPhi) > M_PI+1e-3){
    std::cerr << "The provided base mounting angles exceed abs(" << M_PI << ")! Skipping identification initialization.";
    return false;
  }
  Eigen::VectorXd Xb_ = Eigen::Map<Eigen::VectorXd>(Xb.data(), Xb.size());
  // Xb_ = Xb.data();
  dyn_ = std::make_shared<panda_identification::Dynamics>(mountRho, mountPhi, Xb_);
  obs_ = std::make_unique<panda_identification::Observers>(dyn_);
  return(dyn_ && obs_);
}
