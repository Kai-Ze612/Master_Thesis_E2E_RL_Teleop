...
bool FrankaMjHardwareSystem::initSim(
    ...
    for(size_t i = 1; i <= robot_count_; i++){
        ...
        // Initialize the identification Obs and Dyns
        try{
          std::stringstream id_param_ss(info_.hardware_parameters.at("id_params" + suffix));
          std::vector<double> Xb;
          std::string temp;
          while(std::getline(id_param_ss, temp, ',')){
            Xb.push_back(std::stod(temp));
          }
          // Xb[0] = mounting angle 1, Xb[1] = mounting angle 2, Xb[2:] = identification parameters
          arm.robot_->has_identification_ = arm.robot_->initializeIdentification(std::vector<double>(Xb.begin()+2, Xb.end()), Xb[0], Xb[1]);
          if(!arm.robot_->has_identification_){
            RCLCPP_WARN(getLogger(), "Identification dynamics and/or observers could not be initialized for %s", mj_robot_name.c_str());
          }
          else{
            RCLCPP_INFO(getLogger(), "Identification dynamics and observers have been initialized for %s", mj_robot_name.c_str());
          }
          // Now make a copy of the dyn_ shared pointer to the Model
          arm.robot_->franka_hardware_model_->setDynamics(arm.robot_->dyn_);
        }
        catch(std::exception &e){
          RCLCPP_FATAL(getLogger(), "Identification for %s could not be initialized: %s", mj_robot_name.c_str(), e.what());
          arm.robot_->has_identification_ = false;
        }
  ...
  */
  ...
  for(auto& arm : arms_){
    ...
    if(stop_type > 0){
      ...
      is_effort = all_of_element_has_string(arm_stop_interfaces, "effort");
      is_position = all_of_element_has_string(arm_stop_interfaces, "position");
      is_velocity = all_of_element_has_string(arm_stop_interfaces, "velocity");
  ...
  for(auto& arm : arms_){
    ...
    if(start_type > 0){
      ...
      is_effort = all_of_element_has_string(arm_start_interfaces, "effort");
      is_position = all_of_element_has_string(arm_start_interfaces, "position");
      is_velocity = all_of_element_has_string(arm_start_interfaces, "velocity");
      is_duplicate = (is_effort && is_position) || (is_effort && is_velocity) || (is_velocity && is_position);
      if(!(is_effort || is_position || is_velocity)){
