...
#include "franka_hardware/common/identification/Dynamics.h"
...
class ModelSim : public virtual ModelBase{  // NOLINT(cppcoreguidelines-pro-type-member-init,
 ...
 public:
  ...
  void setDynamics(std::shared_ptr<panda_identification::Dynamics> dyn){dyn_ = dyn;}; // we want to expliclty store a copy of dyn pointer
 ...
 private:
  ...
  mjData* data_;
  std::shared_ptr<panda_identification::Dynamics> dyn_ = nullptr;
  ...
  virtual std::array<double, 49> massImpl(
    ...
    try{
      if(dyn_){
        Eigen::Map<Eigen::MatrixXd>(result.data(), 7, 7) = dyn_->get_M(Eigen::Map<const Eigen::VectorXd>(q.data(), q.size()));
        return result;
      }

    }
    catch(std::exception &e){
      std::cerr << "Exception occurred while using Dynamics class' get_M function. "
                   "Returning default value. Exception: " << e.what() << endl;
    }
  ...
  virtual std::array<double, 49> fullCoriolisImpl(
      const std::array<double, 7>& q,        // NOLINT(readability-identifier-length)
      const std::array<double, 7>& dq)       // NOLINT(readability-identifier-length)
      const noexcept override final {
    std::array<double, 49> result{0};
    try{
      if(dyn_){
        Eigen::Map<Eigen::MatrixXd>(result.data(), 7, 7) = dyn_->get_C(Eigen::Map<const Eigen::VectorXd>(q.data(), q.size()), Eigen::Map<const Eigen::VectorXd>(dq.data(), dq.size()));
        return result;
      }
      ...
      std::cerr << "Exception occurred while using Dynamics class' get_M function. "
                   "Returning zero value. Exception: " << e.what() << endl;
    }
    return result;
  }
  ...
  virtual std::array<double, 7> gravityImpl(
    ...
    try{
      if(dyn_){
        Eigen::Map<Eigen::VectorXd>(result.data(), 7) = dyn_->get_tau_G(Eigen::Map<const Eigen::VectorXd>(q.data(), q.size()));
        return result;
      }

    }
    catch(std::exception &e){
      std::cerr << "Exception occurred while calling Dynamics class' get_tau_G function. "
                   "Returning default value. Exception: " << e.what() << endl;
    }
    for(int i = 0; i < 7; i++){
