...
class ModelBase {  // NOLINT(cppcoreguidelines-pro-type-member-init,
 ...
 public:
  ...
  // Extensions from the Identification feature, offered by Dynamics
  /**
   * Calculates the full Coriolis matrix C. Must be implemented using the inherited class's Dynamics variable.
   *
   * @param[in] q Joint position.
   * @param[in] dq Joint velocity.
   *
   ...
   */
  [[nodiscard]] std::array<double, 49> fullCoriolis(
      const std::array<double, 7>& q,        // NOLINT(readability-identifier-length)
      const std::array<double, 7>& dq) const noexcept { // NOLINT(readability-identifier-length)
    return fullCoriolisImpl(q, dq);
  }
  /**
   * Calculates the full Coriolis matrix C.
   *
   * @param[in] robot_state State from which the Coriolis matrix should be calculated.
   *
   * @return Coriolis matrix.
   */
  [[nodiscard]] virtual std::array<double, 49> fullCoriolis(
    const franka::RobotState& robot_state) const {
  return fullCoriolis(robot_state.q, robot_state.dq);
}
 ...
 private:
  ...
  virtual std::array<double, 49> fullCoriolisImpl(
      const std::array<double, 7>& q, // NOLINT(readability-identifier-length)
      const std::array<double, 7>& dq) const = 0; // NOLINT(readability-identifier-length)
};
