...
While the `ModelSim` class' inherited functions all have arguments, they are not used (except for `franka::Frame` arguments), since all the necessary data is contained in `mjData` and `mjModel`. It was simply left as-is for the sake of having identical interfaces.

In the simulated robot, `franka::Frame::kEndEffector` and `franka::Frame::kStiffness` are treated as the same, since that also seems to be the default behavior of the real robot (i.e. both frames coincide with each other).
