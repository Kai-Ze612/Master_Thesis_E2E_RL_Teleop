...
set(_exported_dependencies "mujoco_ros;pluginlib;rclcpp;rclcpp_lifecycle;std_msgs;geometry_msgs;mujoco_ros_msgs;tf2;tf2_ros;tf2_geometry_msgs")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to mujoco_ros2_sensors_DEFINITIONS, mujoco_ros2_sensors_INCLUDE_DIRS,
# mujoco_ros2_sensors_LIBRARIES, and mujoco_ros2_sensors_LINK_FLAGS.
# Additionally collect the direct dependency names in
# mujoco_ros2_sensors_DEPENDENCIES as well as the recursive dependency names
# in mujoco_ros2_sensors_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(mujoco_ros2_sensors_DEPENDENCIES ${_exported_dependencies})
  set(mujoco_ros2_sensors_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(mujoco_ros2_sensors_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(mujoco_ros2_sensors_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(mujoco_ros2_sensors_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(mujoco_ros2_sensors_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(mujoco_ros2_sensors_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND mujoco_ros2_sensors_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
